...
Infinite.prototype.prepare = function () {
    ...
    this._url = window.location.startsWith("/a/") ? "/api/list/" : "/api/lt?tag=" + window.location.pathname.split('/')[window.location.pathname.split('/').length - 1] + "&skip="
...
Infinite.prototype.handleScroll = function () {
    ...
    if (pageHeight - (scrollPos + clientHeight) < this.Constant_.__distance) {
        ...
        this._count++;
        var _this = this;
        fetch(this._url + this._count).then(function (res) {
            return res.text();
        }).then(function (value) {
